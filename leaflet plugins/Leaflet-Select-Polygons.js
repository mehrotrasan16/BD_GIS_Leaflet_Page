...
$.each(tx_counties.features, function(index, feature) {
    var name = `${feature.properties.kind} ${feature.properties.name}  ( ${feature.properties.state} - )`
    placenames.push(name);
    zipcodes[name] = feature.properties.kind;
});
...
function redraw(b) {
    geojson.eachLayer(function(layer) {
        if (layer.feature.properties.kind == zipcodes[b]) {
            selectTypeaheadFeature(layer)
        }
    })
}

var geojson = L.geoJson(tx_counties, {
...
function addLayers(feature, callback) {
    featuresSelected.push({
        zipcode: feature.properties.kind,
        feature: feature
    })
    callback(arguments[2], arguments[3])
}
...
function checkExistsLayers(feature) {
    ...
    for (var i = 0; i < featuresSelected.length; i++) {
        if (featuresSelected[i].name == feature.properties.name) {
            result = true;
            break;
        }
    ...
    this._div.innerHTML =
        ...
        '<h4>Properties</h4>' + (properties ?
            `
                ...
                Gemeente: ${properties.state}<br>
                Provincie:${properties.name}<br>
                Postcode:${properties.kind}
